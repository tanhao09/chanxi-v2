...
#*****@@@****#
CUR_DIR=$PWD
SCRIPT_DIR=$(dirname $JETTY_SH)
SCRIPT_BASENAME=$(basename $JETTY_SH)
cd $SCRIPT_DIR
SCRIPT_DIR=$PWD
SCRIPT_SH=$SCRIPT_DIR/$SCRIPT_BASENAME
cd $CUR_DIR
JETTY_PID=$SCRIPT_DIR/jetty.pid
...
if [ -f "$SCRIPT_DIR/jetty.conf" ]
then
    JETTY_CONF=$SCRIPT_DIR/jetty.conf
fi
echo $SCRIPT_DIR/jetty.conf
